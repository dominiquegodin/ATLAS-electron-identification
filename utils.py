...
def make_sample(data_file, var_dict, idx, float16=True, upscale=False, denormalize=False):
    ...
    sample = dict([key, data[key][idx[0]:idx[1]]] for key in var_list if 'image_tracks' not in key)
    if 'image_tracks' in var_list: sample.update({'image_tracks':data['tracks'][idx[0]:idx[1]]})
...
def make_labels(sample, n_classes):
    ...
    #    labels = np.where(np.logical_and(MC_type!=2, MC_type!=4),  1, MC_type)
    #    labels = np.where(MC_type==2                            ,  0, labels )
    #    return   np.where(MC_type==4                            , -1, labels )
    elif n_classes == 5:
        ...
        return   np.where(IFF_type==10                              ,  4, labels  )
    elif n_classes == 6:
        labels = np.where(np.logical_or (IFF_type<= 1, IFF_type== 4), -1, IFF_type)
        labels = np.where(np.logical_or (IFF_type== 6, IFF_type== 7), -1, labels  )
        labels = np.where(IFF_type==2                               ,  0, labels  )
        ...
        return   np.where(IFF_type==10                              , -1, labels  )
    elif n_classes == 9:
...
def analyze_sample(sample, scan=False):
    ...
    print('IFF AND MC TRUTH CLASSIFIERS SAMPLE COMPOSITION (sample:', len(MC_type), 'e)')
    dash    = (26+7*len(MC_list))*'-'
    print(dash, format('\n| IFF \ MC |','10s'), end='')
    for col in MC_list:
        print(format(col, '7d'), end='   |  Total  | \n' + dash + '\n' if col==MC_list[-1] else '')
    for row in IFF_list:
        print('|', format(row, '5d'), '   |', end='' )
        for col in MC_list:
