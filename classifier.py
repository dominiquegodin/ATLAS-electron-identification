...
from utils     import make_sample, sample_cuts, make_labels, compo_matrix, find_bin, sample_weights, get_bin_indices#, generator_sample, Batch_Generator
...
if args.generator == 'ON':
    ...
    y_probs = model.predict_generator(test_generator, verbose=1, workers=n_cpus, use_multiprocessing=True)
else:
    y_true = test_labels
    y_probs = model.predict(test_data)
print('\nCLASSIFIER: best test sample accuracy:', format(100*valid_accuracy(y_true, y_probs), '.2f'), '%')
compo_matrix(y_true, train_labels, y_probs)
if args.plotting == 'ON':
    if args.n_epochs>0: plot_history(training,file_name=args.output_dir+'history.png')
    plot_distributions_DG(y_true, y_probs)#,output_dir=args.output_dir)
